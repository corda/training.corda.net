...
description: An example is not the only way to do things
...
Find the `TokenContract` code [here](https://github.com/corda/corda-training-code/blob/master/020-first-token/contracts/src/main/java/com/template/contracts/TokenContract.java).
...
* Kotlin makes classes `final` by default, and this behaviour is copied here.
...
```
The `.collect(Collectors.toSet())` gets rid of duplicates so that we keep shorter collections. You may wonder why the `.map(it -> it.getHolder().getOwningKey())` lambda is used instead of:
...
As you go through them, you will notice that each aspect is tested:
...
And of course, there is a complex transaction example that passes.
