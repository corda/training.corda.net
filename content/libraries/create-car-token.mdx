...
description: Where we create something new with the SDK
slug: libraries/create-car-token
---

import HighlightBox from "../../src/HighlightBox"

In a previous chapter, you learned about the evolvable token type. You are going to use that knowledge here as part of learning about the Tokens SDK, by preparing a car token. Not only is this a preparation for the next exercise but also for the evolution of your project. The result of this exercise can be found [here](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java), but it is recommended that you do not peek until you have worked on your own.
...
3. Rookie: when you want to see a solution, look over [here](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java).

### Evolvable `CarTokenType`

Ideally, you worked on your own before following along here.

Following the theory from the previous chapter, what parameters would you put in your `CarTokenType`? Feel free to add the ones you want. If you chose to work with Kotlin and made it a `data class`, well played. It will be more concise. Otherwise, here is our choice of fields, in Java:
...
1. the wrapping flow defined here: [`IssueCarTokenTypeFlow`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/main/java/com/template/car/flow/IssueCarTokenTypeFlow.java),
2. its usage with specific values and nodes in the unit test class: [`createNewBmw()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L57-67),
3. its output confirmation in the unit test proper: [`@Test isCreated()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L106-113).

You must think that the `IssueCarTokenTypeFlow` flow is not particularly useful, and that would be right as it really just does minimal actions prior to launching [`CreateEvolvableTokens`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/main/java/com/template/car/flow/IssueCarTokenTypeFlow.java#L49) proper. It provides a sense of what to expect when working with meaningful flows.
...
1. the wrapping flow: [`IssueCarToHolderFlow`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/main/java/com/template/car/flow/IssueCarToHolderFlow.java), which needs to be passed information that came from the DMV,
2. its usage with specific values: [`issueCarTo()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L70-78),
3. its output confirmation in the unit test proper: [`@Test isIssuedToAlice()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L116-135).
...
```
Which, in our case, we split into 3 parts:

1. the wrapping flow: [`UpdateCarTokenTypeFlow`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/main/java/com/template/car/flow/UpdateCarTokenTypeFlow.java),
2. its specific usage: [`updateMileageOn()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L81-90),
3. its output confirmation in the unit test proper: [`@Test isIssuedThenUpdated()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L138-162), which confirms that Alice did not automatically get the update.

### Sell the car to Bob

Now that the car has been issued and the token updated, it is time to sell the car to Bob. This is rather simple, given the previous information we have, which again, in this rootless code, is supposed to be travelling from one node to the other:
...
1. the wrapping flow: [`MoveCarToNewHolderFlow`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/main/java/com/template/car/flow/MoveCarToNewHolderFlow.java),
2. its specific usage: [`moveCarTo()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L93-103),
3. its output confirmation: [`@Test isIssuedUpdatedAndSoldToBob()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L165-193).
...
1. Using `QueryCriteria`:
    ...
    ```
    See [`@Test canAccessByUUIDAndQuery()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L200-227) for the example.
2. Or using a `LinearPointer`:
    ...
    ```
    See [`@Test canAccessByUUIDAndPointer()`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/car/flow/CarTokenCourseExercise.java#L230-255) for the example.

### Observer nodes

All the flows that come with the SDK accept a list of observer nodes.

Observer nodes are parties which are not participants in the state but want to be aware of what's happening in your network (e.g. auditors or regulators that require read-only access); so when the transaction gets finalised, it gets recorded in the participants and the observer nodes. The SDK also provides `ObserverAwareFinalityFlow` for the same purpose.
...
6. Remember when you query for a balance to always filter by token type and by issuer, because as we mentioned earlier, tokens with the same token type but different issuers are considered different. [`QueryUtilities`](https://github.com/corda/token-sdk/blob/master/workflows/src/main/kotlin/com/r3/corda/lib/tokens/workflows/utilities/QueryUtilities.kt) has a lot of useful functions that will make your life easier, including a function to calculate the balance for a certain token type and issuer: `VaultService.tokenBalanceForIssuer`:
