...
description: Or how to increase Corda's reach
...
* Even though `AccountInfo` is of type `LinearState`, it has only one command: `Create`. For context, when the library was first introduced, the `AccountInfo` state had a `status` attribute, without any expected `Update` command, so the attribute was useless and subsequently removed from the state.
...
```
See it in action in the course unit test [`AccountVaultCourseExercise`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/040-accounts-lib/040-accounts-lib/workflows/src/test/java/com/template/account/AccountVaultCourseExercise.java):

* When a node hosts an account and has a state in which the account participates, then it can retrieve it from the vault by id, see [`canFetchStateOfSeenWhenKnowsAccount`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/040-accounts-lib/040-accounts-lib/workflows/src/test/java/com/template/account/AccountVaultCourseExercise.java#L78).
* When a node hosts an account and has a state in which the account does not participate, then it cannot retrieve it from the vault by id, see [`cannotFetchStateOfLostWhenKnowsAccount`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/040-accounts-lib/040-accounts-lib/workflows/src/test/java/com/template/account/AccountVaultCourseExercise.java#L153).
