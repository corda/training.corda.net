...
We did not use any of those either, but here is an example of what [could have been](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/contracts/src/main/java/com/template/states/TrialToken.java):
...
```
already exists as [`interface FungibleAsset<T>`](https://github.com/corda/corda/blob/68bb7a0e7bb900117c2ed0d9174fea36d3d4aedc/core/src/main/kotlin/net/corda/core/contracts/FungibleAsset.kt#L31). It's amusing that even in `TrialToken` above, where existing classes were wisely reused, it just so happens that `FungibleAsset` was reinvented. Let's make use of this class in [`QuickAirMileToken`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/contracts/src/main/java/com/template/states/QuickAirMileToken.java):
...
```
that seems to achieve what your `TokenContract` achieved. Poring through its code is more involved than the quick examples above, but you can recognise helper functions in its `Companion` object:
...
```
You can create your contract [`QuickAirMileContract`](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/contracts/src/main/java/com/template/contracts/QuickAirMileContract.java) now:
...
```
But let's stop here, because implementing `verify` is still as much work as we did earlier.
