...
description: Where we describe how we modified our project to use the SDK
slug: libraries/how-we-used-tokens-sdk
---

import HighlightBox from "../../src/HighlightBox"

You will get the most from this example if you compare to your own attempt. Did you refactor your project to use the SDK? Compare this example to your attempt. The code can be found [here](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/tree/030-tokens-sdk/030-tokens-sdk).
...
## The [air-mile type](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/blob/030-tokens-sdk/030-tokens-sdk/contracts/src/main/java/com/template/states/AirMileType.java)
...
```
Why use a static constructor and not make it a proper class? Good question. This is to avoid having to handle complications surrounding `FungibleToken`. We skipped the _detail_ that the `FungibleToken` constructor wants the [hash of the JAR](https://github.com/corda/token-sdk/blob/1.1/contracts/src/main/kotlin/com/r3/corda/lib/tokens/contracts/states/FungibleToken.kt#L38) file that hosts your token type. With our `static` constructor, lazily doing:
...
```
Of course, do not forget the `hashCode` and `equals` functions that ensure an `AirMileType` instance is no different from another. This `getContractAttachment()` function is the one we will need to use when instantiating `FungibleToken`s.

You will notice a `DummyContract`, to satisfy the basics of a "Contracts" CorDapp.

## The [contract _learning_ tests](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/tree/030-tokens-sdk/020-first-token/contracts/src/test/java/com/template/contracts)
...
Thanks to `tweak` we have a high assurance that the error we thought we tested is what we tested indeed. Having it all in a single test is more encapsulated than having 2 individual tests testing both scenarios. Those 2 individual tests might be modified independently by a developer who may not fully grasp the connection between the 2.
...
Here too the specifications have changed, and tests need to reflect that: `@Test redeemTransactionMustHaveLessInOutputs()`. The outputs are understood as the _change_ (as in "Do you have _change_ for a &#36;20 bill?") given when redeeming states with a quantity larger than desired. Remember the `RedeemFlows.SimpleInitiator`? It did a move transaction, in order to reorganise the states, before a redeem if the collected quantity was too high. Here, with the _change_ mechanism, the Tokens SDK allows for a single redeem transaction.

## The [flows](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/tree/030-tokens-sdk/030-tokens-sdk/workflows/src/main/java/com/template/flows)
...
## [Flow tests](https://git.b9lab.com/client-projects/r3-corda-education-services/r3-course-project/tree/030-tokens-sdk/030-tokens-sdk/workflows/src/test/java/com/template/flows)
