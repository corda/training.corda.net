...
By the way, you can extend `TokenType` and add your own custom attributes. Be careful though, as once you instantiated your `TokenType`, you cannot change the value of its attributes! For instance a `CryptoKitten` token-type would look like this:

![Extended TokenType](images/cryptokitten.png)

If you first instantiate a `CryptoKitten` with `tokenIdentifier: BengalCatType`, you would not expect this very instance to suddenly be changed to `tokenIdentifier: PersianCatType`. To have _a Persian_, you would need to instantiate an entirely new `CryptoKitten` instance.
