...
To create your evolvable unit, you need to extend `EvolvableTokenType` and add your custom attributes. For our example, you can represent a car unit as a `CarTokenType`. Remember, you first need to define a unit. Only then can you issue tokens of that unit. The token type will have updateable attributes, price and mileage, and fixed attributes, make and VIN number.
...
* as we have seen, `IssuedTokenType` has an `issuer` and a `tokenType` of type `TokenType`.

So graphically, that's:

![NonFungibleToken composition](images/non_fungible_token.png)

Notice in the above example how the `issuer` and `maintainers` are different entities declared in different classes. Let's suggest a series of steps that could take place in our car example:
