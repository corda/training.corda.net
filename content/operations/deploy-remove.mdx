...
1. Make sure that there are no running flows that use the to-be-deleted CorDapp, by [draining](https://docs.corda.net/docs/corda-os/4.3/upgrading-cordapps.html#flow-drains) your node. If the flow on your node is a result of responding to a flow from another node, the initator flow must be killed as well. Otherwise it will be stuck waiting for your node's response.
