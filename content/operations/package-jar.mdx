...
If you recall, a signed transaction is constituted of its content, its serialized bytes, which determine its id. When a signature is added to a transaction, it is indeed _added_ to it, the signature does not change the transaction bytes or id. Similarly here, when you sign your CorDapp, you do not modify its content, instead you affix one or more signatures to it.
...
Now, to the contracts CorDapp. Gradle will need to know:

* Where the PKCS12 file is.
* What password to use to decrypt it.

And because neither the file, nor the password shall be committed into the Git repository, you need to create a local-only configuration file. So create a `signing.properties` file in your project folder and add it to `.gitignore`. In your project's folder:
...
```
Where you have replaced `/absolute/path/to/` with the correct absolute path to the key, don't use `~`, and put the correct password. Now, it is time to direct Gradle how to sign.

### Use the key to sign

Add the below code inside the `build.gradle` file of each contracts CorDapp in your project. Notice how it makes use of `getProperty()` instead of exposing the plain text passwords:
...
cordapp {
    ...
    signing {
        ...
        options {
            ...
            storepass signing.getProperty("jar.sign.password")
            keypass signing.getProperty("jar.sign.password")
            storetype "PKCS12"
        }
    }
}
```
Now that Gradle is correctly informed, it is time to package it again:
