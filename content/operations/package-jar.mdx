...
If you recall, a signed transaction is constituted of its content, its serialized bytes, which determine its id, and signatures. When a signature is added to a transaction, it is indeed _added_ to it and the signature does not change the transaction bytes or id. Similarly here, when you sign your CorDapp, you do not modify its content, instead you affix one or more signatures to it.
