...
The decision was taken to not alter the existing `SalesProposal` and contract, and leave them as they are. Adding complexity to something that works well is always fraught. The DMV's _lien_ oracle will be handled by other, new, elements.

Another decision taken is that the buyer could singlehandedly decide to have a verification done at the time of purchase, or involve the seller in it as well. Also, the ledger constraints are designed to allow purchases even if there are liens. The control of the desired situation is left to flows. It is a business decision.
...
## [`DueDiligenceContract`](TODO-link)
...
Evidently, `Status` needs to be defined:
...
As for the [contract constraints](TODO-link), beside the classic ones, they are:
...
The [tests](TODO-link) test these few constraints in turn.

## [DiligenceOracleUtilities](TODO-link)

Defining those early on can clear some future mental load. When the oracle informs about a status, it has a certain validity period:

```java
public static final Duration VALID_DURATION = Duration.ofMinutes(10);
```
Yes, it means that the oracle will check the time-window, which leads to the way transactions will be filtered:
...
```
Note how the predicate selects the `TimeWindow`.

Time to move to the oracle proper.

## [`DiligenceOracle`](TODO-link)
...
```
And for simplicity's sake, we assume that it keeps a mapping in memory:
...
```
Where it assumes that absence is proof of absence, the perks of being a centralised authority. What about the sign function?
...
```
Looks like a copy-pasta of what you learned, with the additional check that the key needs to have been set. What is this `isCommandWithCorrectParameters`:
...
```
It must look familiar to you. It accepts a `Certify` command with valid parameters:

> `cmdData.getStatus().equals(query(cmdData.getTokenId()))`

Here you can see that it can certify either that the vehicle is clear or that it is linked. Oh and it checks, via the time-window, that the transaction will be notarised before the status expires.
...
## [`DiligenceOracleInternalFlows`](TODO-link)

To assist in controlling the state of the oracle, a couple internal flows were created. They are meant to run on the same node of the oracle and do not have handling counterpart:
...
```
It only accepts to sign as a participant if it can find that it owns a token with a matching id:
...
```
That should ward off spam.

## [`Drop` flows](TODO-link)

Yes, you need to go through all the basic stuff before munching on the interesting bits. These flows are the mirror image of the `Prepare` flows. Expectedly, the handler checks that it is a participant in the input `DueDiligence`. That's a strong check, because you have to sign to become a participant in the first place.

## [`Certify` flows](TODO-link)

### The `Request` / `Answer` pair
...
* Filter the transaction (this runs on the requester's host).
...
### The `RequestStraight` / `FinaliseStraight` pair

Ok so far, if you are meticulous, you will notice that there has been no flows that does a proper `Certify` transaction, one where the oracle's opinion is used for real. So far it has all been data passed around.
...
```
Glad that the `Request` flow exists to chip away at the boilerplate that would otherwise be here. Then, it is time to add the signature to the transaction:

```java
final SignedTransaction signed = new SignedTransaction(wtx, Collections.singletonList(oracleSig));
```
That's right, this node does not need to sign. Only the oracle signature is required, so you might as well built your `SignedTransaction` out of its elements...
...
Check out the [tests](TODO-link) too.
...
Remember that the `SalesProposal` state and contract were not modified to facilitate this `DueDiligence` thingy. They are 2 separate entities and have no knowledge of each other. This separation of concern at the ledger level in no way forces you to a symmetrical separation of concern at the business layer, i.e. flows.
...
## [Modified `SalesProposalAcceptFlows`](TODO-link)
...
```
With the additional constraint that if `dueDiligenceId == null`, then `diligenceStatus` is not even looked at. This loose coupling is somewhat expressed through public constructors that take both of them, or none of them.
...
* The match between the token id and the asset id is done here, in the flow.
* The time-window is made more restrictive if necessary. There is a single time-window, as expressed by the name `setTimeWindow`, so the code conditionally overwrites.
...
```
And when it's time to sign with our keys:
...
Check out the [new test file](TODO-link) for the case when there is a `DueDiligence`.
