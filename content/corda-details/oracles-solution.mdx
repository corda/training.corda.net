...
Moving on. For simplicity's sake, we assume that it keeps a mapping in memory:
...
## [`DueDiligenceOracleFlows`](TODO-link)
...
```
With the additional constraint that if `dueDiligenceId == null`, then `diligenceStatus` is not even looked at. This loose coupling is somewhat expressed through public constructors that [take both](TODO-link) of them, or [none](TODO-link) of them.
