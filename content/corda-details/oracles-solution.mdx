...
Defining those early on can minimise future mental overhead. When the oracle informs about a status, it has a certain validity period:
...
```
That's right. This node does not need to sign. Only the oracle signature is required, so you might as well build your `SignedTransaction` out of its elements...
...
Check out the [tests](TODO-link) as well.
