...
* `SalesProposal` state found [here](TODO-link).
* `SalesProposalContract` contract, with changes for all 3 commands `Offer`, `Reject` and `Accept`, found [here](TODO-link).
* The 3 flow pairs found [here](TODO-link):
...
Therefore, the only addition is, found [here](TODO-link):
