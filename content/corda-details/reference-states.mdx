...
description: When you thought simple inputs was all there was
slug: details/reference-states
---

import HighlightBox from "../../src/HighlightBox"

So far in this training, when the subject of states has been mentioned, you have read and read again that they can be consumed as transaction inputs and can only be consumed once. That is still true and your world is not going to collapse. When a state is data that evolves, like tokens changing hands or an obligation that gets refunded, then it makes perfect sense to consume and move on. Plus we avoid double-spending.
...
## What are options for reference data?
...
```
Find the incomplete `KYCContract` [here](TODO-link). Incomplete, as it leaves missing pieces to you as an exercise.
