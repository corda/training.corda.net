---
title: Run Example
description: Run the example app and see the db
...
#### Did you get this error?
    ...
    ```
    If yes, look back at the "Prepare your computer" chapter, and look for the part where we describe how to run Gradle with a `JAVA_HOME` that differs from your default one. That may mean running something like:
...
Now that our nodes have been deployed, let us do as per what the [documentation says](https://docs.corda.net/tutorial-cordapp.html#id1) and run this command:
...
java.lang.IllegalArgumentException
        ...
        at co.paralleluniverse.asm.ClassReader.<init>(Unknown Source)
```
If yes, look back to the "Prepare your computer" chapter, and look for the part where we describe how to run Gradle with a `JAVA_HOME` that differs from your default. That may mean running something like:
...
<!--
* the `Database connection url` which we will use in H2 console later to explore each node's vault
-->
...
We are going to create our first IOU where PartyB owes 50 to PartyA. Because of how the flows are coded:

1. we need to initiate the flow from the lender, i.e. PartyA.
...
It takes a few seconds. That is because our processes are started lazily. Notice the last line, that looks like:
...
* and a single state was created, or `produced`, with the expected values. This concludes our first interaction with a flow from the command line.

Watch a step-by-step screencast of this process:

`video: https://youtu.be/YdvTIQLz7l8`

In the next chapter, we will do the same but look deeper into the database.
