...
Shall we peak into the databases of the nodes and see in detail what was stored? The problem is, the database is not accessible with an external tool. We need to do some additional configuration steps.

The database used in a development network is H2. To access it, we need to:

* install some H2 software
* reconfigure our nodes
...
As we mentioned earlier, each node terminal displays the `Database connection url `, so if you want to connect to a given node just copy the URL and paste it in `JDBC URL` in H2's login console. Don't change anything else. Click "Connect".
...
Let us start by connecting to our notary on our first H2 tab. Click on `NODE_NOTARY_REQUEST_LOG`:
...
Now let us run the same `*` query on another table: `SELECT * FROM IOU_STATES `. Here again, we have a single state:
...
Let us turn our attention from the database to PartyA's interactive shell:

```yaml
>>> run vaultQuery contractStateType: com.example.state.IOUState
```
We get a composite object of `IOU_STATES`, `NODE_ATTACHMENTS`, `VAULT_STATES` and `VAULT_LINEAR_STATES_PARTS`.
...
```
We get the same list as `flow list`.

[Here](https://github.com/corda/corda/blob/67e3e09a4edb41d17b8adfa669d671c690c5db33/node/src/main/kotlin/net/corda/node/internal/CordaRPCOpsImpl.kt#L299) we see we can ask:
...
We will still use PartyA as the lender, but PartyC will now be the borrower. This time, we are going to experiment with a checkpointed state machine. You will remember that a checkpoint happens when a flow is waiting for something coming from elsewhere. In the IOU case, PartyA has to wait for PartyC to return the transaction it signed. It is at this point that PartyA will checkpoint. PartyA will checkout whether PartyC is running or stopped.

However, for PartyA's checkpointed state to last long enough for us to see it, we will stop PartyC. Type `bye` into PartyC's shell, but do not close the terminal so that you can relaunch it later. While you are at it, also stop the Notary.
...
Do not interrupt PartyA's flow and let us return to our H2 console connected to PartyA. And run the following queries:
...
<!--
Using the up arrow on the keyboard should do the trick to find the line with which it was started. Otherwise, no sweat, w

No, it doesn't because they have not started an individual node before this point. So we should not say it.
-->

Ok, let us start PartyC again. We will start the individual node from the `workflows-java/build/nodes` folder that was created by `./gradle deployNodes`.
...
```powershell
C:\example-cordapp> cd workflows-java/build/nodes/PartyC
...
java.lang.IllegalArgumentException
        ...
        at co.paralleluniverse.asm.ClassReader.<init>(Unknown Source)
```
If yes, look back at the "Prepare your computer" chapter, and look for the part where we describe how to run a JAR with a different version from your default. That may mean running something like:
...
We have seen that the notary was not contacted on creation of the IOU. And, since we have yet no flow that consumes an input state, we have no way of observing the notary's inner workings.

What if we said the notary should also be `validating`? In `workflows-java/build.gradle`:
...
Here you need to close all the previously launched nodes. That is 4 terminal tabs in all. You should also close you H2 Console browser tabs because the nodes are configured to use random database ports. Then:
...
We have covered a lot of ground. This step-by-step screencast demonstrates the exercise up to this point:

`video: https://youtu.be/KAtDzwyrEfk`

### Potential `.jar` injection?

Ok, so our `Acceptor` flow can be triggered remotely by a party that launches the `Initiator` flow. Is this so mechanical? We saw that our `nodes/PartyA/cordapps/cordapp-example-contracts-0.1.jar` file was referred to in the H2 database as an attachment. So now, let us redeploy nodes and remove this jar file from PartyB:
...
Now in PartyA, run `>>> flow start ExampleFlow$Initiator iouValue: 50, otherParty: "O=PartyB,L=New York,C=US"`. We get:
