...
* **Inlined** or **free-riding pair**, whereby the _responder_ is not necessarily automatically awoken by the _initiator_, but instead it is the responsibility of the encapsulating flows to both launch the initiator and the responder at the right point in the choreography, as you'll see with `CollectSignaturesFlow`. Inlined flows always take at least one flow session among their constructor parameters.
