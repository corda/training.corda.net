...
description: With some Java preparation
...
<ExpansionPanel title="Corda has a Web-accessible online IDE">

This online IDE that lets you do surprisingly a lot. Access it at [ide.corda.net](https://ide.corda.net/). If you have worked with VisualStudioCode before, you will recognise it as an incarnation of its [Web-based version](https://github.com/cdr/code-server).

When you connect for the first time, it will create a unique address for you, like `abcdef.ide.corda.net`. If it times out the first time, give the [Docker container](https://www.docker.com/resources/what-container) some time to start and try again after a minute.

Do not use it for your production code, though. You are not in control of the Docker container's lifecycle.

Use this facility as an extra, not as a way to avoid doing the hard work of getting your IDE up to scratch.
...
- **Oracle JDK:** This is important. Be sure to install the [Oracle JDK version 8](https://www.oracle.com/java/technologies/javase/javase-jdk8-downloads.html) (not the JRE) at version 8u171 or higher. If you have a newer version of Java installed, you need to install version 8 alongside your newer version. Below, and later in the course, we show how you can point Corda to the desired version.
- **IDE:** You may use any Java-able IDE you like. [IntelliJ IDEA Community Edition](https://www.jetbrains.com/idea/download/#section=mac) is this training's first choice and it will be used throughout. In a later section, you will be introduced to VisualStudioCode; not the Web version.
