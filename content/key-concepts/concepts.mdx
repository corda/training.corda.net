...
![Bilateral Ledger](images/anatomy-bilateral-ledger.png)
...
To help you get accustomed to the Corda way of describing these state transitions, let’s review what happens when Alice sends Bob &#36;1.
...
Flows model the business processes that oversee the evolution of transaction states. Consider the simple example of Alice's IOU. Alice first proposes the IOU which includes reference to the IOU contract that informs the contract state to be created, as well as a proposal that Bob will credit Alice's account with some currency. There is a process of gathering approvals before this proposed transaction is final.
